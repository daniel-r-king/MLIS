...
consc_pca_df = pd.DataFrame(data = consc_pcs, columns = ['pc1','pc2'], index = full_df['gender'].values)

consc_gender_plot = plt.figure(1, figsize = (10,10))
plt.scatter(consc_pca_df['pc1'], consc_pca_df['pc2'], 1, 
color = ['blue' if x == 1
else 'red' if x == 2
else 'purple' for x in consc_pca_df.index])
consc_gender_plot.suptitle('conscientiousness by gender')

pca = PCA(n_components = 2)
open_pcs = pca.fit_transform(open_df)
open_pca_df = pd.DataFrame(data = open_pcs, columns = ['pc1','pc2'], index = full_df['gender'].values)

open_gender_plot = plt.figure(2, figsize = (10,10))
plt.scatter(open_pca_df['pc1'], open_pca_df['pc2'], 1, 
color = ['blue' if x == 1
else 'red' if x == 2
else 'purple' for x in consc_pca_df.index])
open_gender_plot.suptitle('openness by gender')

pca = PCA(n_components = 2)
neuro_pcs = pca.fit_transform(neuro_df)
neuro_pca_df = pd.DataFrame(data = neuro_pcs, columns = ['pc1','pc2'], index = full_df['gender'].values)

neuro_gender_plot = plt.figure(3, figsize = (10,10))
plt.scatter(neuro_pca_df['pc1'], neuro_pca_df['pc2'], 1, 
color = ['blue' if x == 1
else 'red' if x == 2
else 'purple' for x in consc_pca_df.index])
neuro_gender_plot.suptitle('neuroticism by gender')

pca = PCA(n_components = 2)
agree_pcs = pca.fit_transform(agree_df)
agree_pca_df = pd.DataFrame(data = agree_pcs, columns = ['pc1','pc2'], index = full_df['gender'].values)

agree_gender_plot = plt.figure(4, figsize = (10,10))
plt.scatter(agree_pca_df['pc1'], agree_pca_df['pc2'], 1, 
color = ['blue' if x == 1
else 'red' if x == 2
else 'purple' for x in consc_pca_df.index])
agree_gender_plot.suptitle('agreeableness by gender')

pca = PCA(n_components = 2)
ext_pcs = pca.fit_transform(ext_df)
ext_pca_df = pd.DataFrame(data = ext_pcs, columns = ['pc1','pc2'], index = full_df['gender'].values)

ext_gender_plot = plt.figure(5, figsize = (10,10))
plt.scatter(ext_pca_df['pc1'], ext_pca_df['pc2'], 1, 
color = ['blue' if x == 1
else 'red' if x == 2
else 'purple' for x in consc_pca_df.index])
ext_gender_plot.suptitle('extroversion by gender')
