...
consc_pca_df = pd.DataFrame(data = consc_pcs, columns = ['pc1','pc2'], index = full_df['race'].values)

consc_race_plot = plt.figure(1, figsize = (10,10))
...
else 'darkgoldenrod' if x == 8
else 'gray' if x == 9
else 'maroon' if x == 10
else 'lawngreen' if x == 11
else 'navy' if x == 12
else 'magenta' for x in consc_pca_df.index])
consc_race_plot.suptitle('conscientiousness by race')

pca = PCA(n_components = 2)
open_pcs = pca.fit_transform(open_df)
open_pca_df = pd.DataFrame(data = open_pcs, columns = ['pc1','pc2'], index = full_df['race'].values)

open_race_plot = plt.figure(2, figsize = (10,10))
...
open_race_plot.suptitle('openness by race')

pca = PCA(n_components = 2)
neuro_pcs = pca.fit_transform(neuro_df)
neuro_pca_df = pd.DataFrame(data = neuro_pcs, columns = ['pc1','pc2'], index = full_df['race'].values)

neuro_race_plot = plt.figure(3, figsize = (10,10))
...
neuro_race_plot.suptitle('neuroticism by race')

pca = PCA(n_components = 2)
agree_pcs = pca.fit_transform(agree_df)
agree_pca_df = pd.DataFrame(data = agree_pcs, columns = ['pc1','pc2'], index = full_df['race'].values)

agree_race_plot = plt.figure(4, figsize = (10,10))
...
agree_race_plot.suptitle('agreeableness by race')

pca = PCA(n_components = 2)
ext_pcs = pca.fit_transform(ext_df)
ext_pca_df = pd.DataFrame(data = ext_pcs, columns = ['pc1','pc2'], index = full_df['race'].values)

ext_race_plot = plt.figure(5, figsize = (10,10))
...
ext_race_plot.suptitle('extroversion by race')
